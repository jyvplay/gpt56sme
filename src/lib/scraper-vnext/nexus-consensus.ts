export * from "./nexus-consensus.orig";

import { nexusResearch as packageResearch } from "./nexus-consensus.orig";
import { filterRelevantSourcesAny, hardenRetrievalQuery } from "@/lib/debug/retrieval-hardener";

export async function nexusResearch(...args: Parameters<typeof packageResearch>): Promise<Awaited<ReturnType<typeof packageResearch>>> {
  ...
  try { opts?.onDebug?.(`workspace-relevance-gate: nexus accepted ${gate.accepted.length}, rejected ${gate.rejected.length}`); } catch {}
