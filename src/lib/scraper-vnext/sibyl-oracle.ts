export * from "./sibyl-oracle.orig";

import { sibylResearch as packageResearch } from "./sibyl-oracle.orig";
import { filterRelevantSourcesAny, hardenRetrievalQuery } from "@/lib/debug/retrieval-hardener";

export async function sibylResearch(...args: Parameters<typeof packageResearch>): Promise<Awaited<ReturnType<typeof packageResearch>>> {
  ...
  try { opts?.onDebug?.(`workspace-relevance-gate: sibyl accepted ${gate.accepted.length}, rejected ${gate.rejected.length}`); } catch {}
